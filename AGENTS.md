...
| `logout` | organization | — | — |
| `platform_health_check` | organization | — | — |
...
| `update_connector` | organization | — | — |
| `get_oauth_url` | organization | — | — |
| `delete_connector` | organization | — | — |
| `test_connector` | organization | — | — |
...
| `get_project_workspace_status` | organization | — | — |
| `list_project_changes` | organization | — | — |
| `plan_project_materialization` | organization | — | — |
...
| `preview_sql_materialize` | project | — | — |
| `run_sql_materialize` | project | — | — |
...
| `deploy_project` | organization | `project_id`, `instance_id` | `force_redeploy` |
...
### `loxtep_query` — Use — find and query trusted data. Catalog discovery (search, entry detail, domains, tags), analytics SQL over data products (execute_query, list_tables, get_table_schema, get_query_results), and COA-shaped graph serve ops backed by DynamoDB SPARQL (translate_sparql, graph_traversal — never Neptune). For trust signals (lineage evidence scores), use loxtep_observe. COA list_metrics/describe_schema are not separate tools — use search_semantic_layer / get_table_schema / list_ontology_concepts.
| Operation | Scope | Required | Optional |
| --- | --- | --- | --- |
...
| `translate_sparql` | organization | — | — |
| `graph_traversal` | organization | — | — |

### `loxtep_observe` — Use — monitor trust, health, lineage, and runtime. Quality scores, catalog evidence/lineage/governance flags, compounding metrics, live queue inspection (get_queue_info, read_queue_events, replay_events, trigger_bot), Iceberg table health/rebuild (get_iceberg_health, rebuild_iceberg_table — confirm required; dry_run first), and deployment status (list_deployments, get_deployment).
| Operation | Scope | Required | Optional |
| --- | --- | --- | --- |
...
| `get_iceberg_health` | organization | `data_product_id` | — |
| `rebuild_iceberg_table` | organization | `data_product_id`, `confirm`, `backfill_from` | `start_checkpoint`, `end_checkpoint`, `dry_run` |
