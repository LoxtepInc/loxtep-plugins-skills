...
If an SDK connector already exists (created via UI, API, or MCP — see the **`connect-external-system`** Agent-Scope Skill), export its `sdk_config` directly:
...
> To create an SDK connector in the first place, see the **`connect-external-system`** Agent-Scope Skill (Flow — SDK Connector).
