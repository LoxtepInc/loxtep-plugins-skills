...
**Loxtep is the AI-first, governed data layer for your business. Build agents in Cursor; Loxtep ingests your data, gives them trustworthy context, and governs every access over hosted MCP.**
...
4. **Use the tools** — In the MCP palette you will see grouped tools like `loxtep_workspace` and `loxtep_build`. Pass **`operation`** (e.g. `list_projects`, `create_data_product`) and the fields that action needs.
